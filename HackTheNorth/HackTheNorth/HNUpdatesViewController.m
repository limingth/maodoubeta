...
- (void)viewDidLoad {
    ...
    UIBarButtonItem* leftItem = [[UIBarButtonItem alloc] initWithTitle:@"筛选" style:UIBarButtonItemStyleDone target:self action:@selector(sponsorButtonPressed)];
    self.navigationItem.leftBarButtonItem = leftItem;
    
    UIBarButtonItem* rightItem = [[UIBarButtonItem alloc] initWithTitle:@"创建" style:UIBarButtonItemStyleDone target:self action:@selector(mapButtonPressed)];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(![tagDict isEqual: [NSNull null]])
    {
        if([tagDict objectForKey:@"name"])
            cell.name = @" 操作系统专题"; //[tagDict objectForKey:@"name"];
        ...
        if([tagDict objectForKey:@"description"])
            cell.message = @"操作系统是计算机系统中负责管理各种软硬件资源的核心系统软件，掌握操作系统的基本原理及其设计实现技术是研究型大学计算机专业本科毕业生的基本要求。\n\n专题发起人：陈渝"; //[tagDict objectForKey:@"description"];
        
        if([tagDict objectForKey:@"avatar"])
            //cell.avatarImgURL = [NSURL URLWithString:[tagDict objectForKey:@"avatar"]];
            cell.avatarImgURL = [NSURL URLWithString:@"http://maodou.io/assets/class_os/class_os.jpg"];
